...
* @icon fa fa-circle-o
*/
class HlybandExt extends HlybandBase
{
	...
	public function idencheckindex() {
		if ($this->request->isPost()) {
			...
			if ($params) {
				...
				$vo = Db::table('qw_hlyband')->where($filter)->find();
				if (!$vo) {
					$bid = Db::table('qw_hlyband')->insertGetId($data);
				}  else {
					$f = Db::table('qw_hlyband')->where($filter)->update(['updated_at' => $data['created_at']]);
					$bid = $vo['id'];
					unset($vo);
				}
...
//					$action = 'renew';
	...
	public function addressSearchindex() {
	...
	public function address() {
		...
		if ($this->request->isAjax()) {
			...
			//第一步信息，从数据库中获取
			$idenInfo = Db::table('qw_hlyband')->field('accnbr,custname,region,icno,type,comments')->where(['status'=>'1','id'=>$bid])->find();
